...
> To set up App Configuration and Custom Field, refer to the [`Configure App Configuration and Custom Field`][app-config-custom-field] section and to configure JSON RTE refer [`Configure JSON RTE`][json-rte] section.
...
To configure these locations, update the files in the [`<APP_DIRECTORY>/ui/src/root_config`](./ui/src/root_config/index.tsx) directory.

This directory holds all configuration logic for defining how UI elements of your app should behave.
...
| SELECTOR_PAGE_LOGO\*              | svg code of Logo | --                                       | svg code of the logo to be added to the Selector Page.                                                                                                  |
| REQUIRED_CONFIG_FIELDS            | array of strings | --                                       | Array of config names. Used for config field validations on ConfigScreen.                                                                               |
| SELECTOR_CONFIG_CHECK_FIELDS      | array of strings | --                                       | Array of config field parameter names. Used for null config checks on the selector page and display error message if any values are missing.            |
| IS_DAM_SCRIPT\*                   | boolean          | --                                       | Determines if Distribution script url is present or not. If `false`, the [`Custom DAM Selector Component`][custom-dam-selector-component] flow is used. |
| DAM_SCRIPT_URL                    | string           | --                                       | Distribution Script URL provided by the third-party DAM.                                                                                                |
| DIRECT_SELECTOR_PAGE\*            | string           | "novalue", "url", "window", "authWindow" | Determines the flow of the Selector Page. Further details can be found in the flow descriptions below.                                                  |
...
Navigate to the root_config file located at (ui/src/root_config/index.tsx). In this file, modify the following functions related to the ConfigScreen as per your requirements.
...
Use this flow when the third-party DAM application has a Script Url present which can be embeded in the \<script> tag of html page.

Example: `<script src="https://dam.com/dam.js"></script>`

Set the following values in `damEnv`:
...
Implement the functions mentioned below to configure Custom Field and Selector Page in the DAM Script URL Flow.
...
Use this flow when the third-party DAM application has a Window Url present which can be opened directly in browser.
...
Implement the functions mentioned below to configure Custom Field and Selector Page in the DAM Window URL Flow.
...
Use this flow when the third-party DAM application provides a Window Component or Frame that is triggered to open on click of a button.
...
Implement the functions mentioned below to configure Custom Field and Selector Page in the DAM Window Component Flow.
...
Use this flow when the third-party DAM application has API or SDK support for accessing their assets.
...
Implement the functions mentioned below to configure Custom Field and Selector Page in the Custom DAM Selector Component Flow.
...
Use this Flow when the third-party DAM application uses an authentication window and needs to open another window—via API or SDK support—to access its assets.
...
Implement the functions mentioned below to configure the Custom Field and Selector Page in the Auth Window Flow.
...
- #### `configureConfigScreen` Function
  ...
  > Note:
  >
  > - The type property can be one of the following:`textInputField | selectInputField | radioInputField| customInputField`.
  > - The property name used (e.g., textField, selectField in the example) will serve as the identifier for the corresponding configuration field being added.
...
- #### `customWholeJson` Function
  ...
  1. customJsonOptions: A list of options to be included in the custom fields dropdown.

  2. defaultFeilds: A subset of customJsonOptions specifying which options should be selected by default.
...
- #### `filterAssetData` Function
  ...
  This function filters the required asset properties and constructs an assetObject, which is used for internal asset management.
...
- #### `handleConfigtoSelectorPage` Function
  ...
  This function manages multiple configuration sources (e.g., the configuration from ConfigScreen and the configuration from the Custom Field Advanced Options), and facilitates the selection of a specific configuration to be passed to the selector page.

  Function Parameters:

  | Name              | Type   | Description                                                            |
  | ----------------- | ------ | ---------------------------------------------------------------------- |
  | config            | Object | Configuration object received from the ConfigScreen                    |
  | contentTypeConfig | Object | Configuration object received from the advanced options of CustomField |
...
- #### `getSelectorWindowUrl` Function
  ...
  This function retrieves the URL of the available third-party DAM window, which is then opened in a pop-up window.
...
- #### `handleSelectorPageData` Function
  ...
  This function handles the data received from the selector page by extracting it from the message event parameter, making it available for use within the application.
...
- #### `handleSelectorWindow` Function
  ...
  This function is triggered when the "Add Asset" button is clicked and is responsible for opening the third-party DAM window. It should include the code logic required to open the DAM window.
  ...
  | config            | Object                        | Configuration object received from the ConfigScreen                    |
  | contentTypeConfig | Object                        | Configuration object received from the advanced options of CustomField |
  | setError          | (errObj: TypeErrorFn) => void | Function to set an error message on the selector page                  |
...
- #### `handleAuthWindow` Function
  ...
  This function is triggered on the "add asset" button click and responsible for opening the third-party Auth window. Once the Auth is successful, it opens the custom selector page window. The function should contain third-party code logic for authentication.
...
- #### `modifyAssetsToSave` Function
  ...
  This function is used to modify the asset data that is being saved in the custom field.
...
- #### `openComptactView` Function
  ...
  This function is used in a DAM Script URL Flow to execute functions from a script source and display a component by attaching it to a specified DOM element reference.

  Function Parameters:

  | Name                                          | Type                          | Description                                                                             |
  | --------------------------------------------- | ----------------------------- | --------------------------------------------------------------------------------------- |
  | config                                        | Object                        | Configuration object received from CustomField                                          |
  | selectedIds                                   | Array of strings              | Selected asset ids from CustomField                                                     |
  | onSuccess                                     | (assets: any[]) => void       | Callback function to send asset data to CustomField. Accepts an array of asset objects. |
  | onCancel                                      | () => void                    | Callback function to close the Selector Page.                                           |
  | { containerRef, containerClass, containerId } | TypeSelectorContainer         | Contains reference for DOM element of the Selector Page                                 |
  | setError                                      | (errObj: TypeErrorFn) => void | Function to set an error message on the selector page                                   |
...
- #### `customSelectorComponent` Function
  ...
  This function is used in Custom DAM Selector Component Flow where the entire selector page is built by the user using APIs or SDKs to fetch and manage the asset data.
  ...
  | config           | Object                        | Configuration object received from CustomField                                |
  | setError         | (errObj: TypeErrorFn) => void | Function to set an error message on the selector page                         |
  | successFn        | (assets: any[]) => void       | Function to send your data to custom field. Accepts an array of asset objects |
...
To configure the JSON RTE plugin for your third-party DAM, you need to modify the files in the [`<APP_DIRECTORY>/ui/rte/src/rte_config`](./ui/rte/src/rte_config/index.tsx) directory.

All related configurations should be defined within this directory. Here, you'll specify how the JSON RTE elements of your app should be handled.
...
| RTE_DAM_ICON\*                    | svg code of Logo icon | --                                       | SVG code of the icon to be added to the JSON RTE actions panel.                                  |
| DIRECT_SELECTOR_PAGE\*            | string                | "novalue", "url", "window", "authWindow" | Determines the flow type of the Selector Page. See the flow descriptions below for more details. |
| ADVANCED_ASSET_PARAMS             | Object                | --                                       | Property used for handling various Asset Constraint Validations                                  |
| ADVANCED_ASSET_PARAMS.ASSET_NAME  | String                | --                                       | Property name for ASSET NAME. If present in nested structure, add nested structure reference.    |
| ADVANCED_ASSET_PARAMS.SIZE_NAME   | String                | --                                       | Property name for ASSET SIZE. If present in nested structure, add nested structure reference.    |
| ADVANCED_ASSET_PARAMS.SIZE_UNIT   | String                | "BYTES"(default), "KB", "MB", "GB", "TB" | Unit for ASSET SIZE provided by third-party dam.                                                 |
...
DAM Script URL Flow is used when the third-party DAM application provides a script URL that can be embedded directly using the \<script> tag in an html page.
...
Implement the following functions to configure JSON RTE within the DAM Script URL Flow:
...
DAM Window URL Flow is used when the third-party DAM application provides a window URL that can be opened directly in the browser.
...
DAM Window Component Flow is used when the third-party DAM application provides a window component or frame that can be triggered to open, typically via a button click.
...
Custom DAM Selector Component Flow is used when the third-party DAM application has API or SDK support for accessing their assets.
...
Implement the functions mentioned below to configure the JSON RTE in the Auth Window Flow.
...
- #### `getDisplayUrl` JSON RTE Function
  ...
  This function retrieves the asset URL to be used as thumbnail of image and for preview and OpenInDAM action of asset.
...
- #### `getSelectorWindowUrl` JSON RTE Function
  ...
  | Name              | Type   | Description                                                          |
  | ----------------- | ------ | -------------------------------------------------------------------- |
  | config            | Object | Configuration object received from ConfigScreen                      |
  | contentTypeConfig | Object | Configuration object received from the advanced options of JSON RTE. |
...
- #### `handleSelectorPageData` JSON RTE Function
  ...
  This function manages the data received from the selector page by extracting it from the message event parameter. The extracted data is then used within the application.
...
- #### `handleSelectorWindow` JSON RTE Function
  ...
  This function is triggered when the "Choose Assets from DAM" button is clicked. It is responsible for opening the third-party DAM window and should include the necessary code logic to open the DAM window.
...
- #### `handleAuthWindow` JSON RTE Function
  ...
  This function is triggered on the "Choose assets from DAM" button click and responsible for opening the third-party Auth window. Once the Auth is successful, it opens the custom selector page window. The function should contain third-party code logic for authentication.
...
- #### `handleConfigtoSelectorPage` JSON RTE Function
  ...
  The function manages multiple Configurations present (i.e Config from ConfigScreen, Config from JSON RTE Advanced Option) and allows configuration selection that needs to be sent to a selector page.
