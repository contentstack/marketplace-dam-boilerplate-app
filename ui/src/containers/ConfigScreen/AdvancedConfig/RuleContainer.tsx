...
const RuleContainer: React.FC<RuleContainerProps> = ({
  ...
  const selectedValues = useMemo(() => {
    ...
    mappings?.forEach((mapping) => {
      ...
      if (isMultiBranch) {
        if (Array.isArray(branchValue)) selectedBranchValues?.push(...branchValue);
      } else {
        if (branchValue) selectedBranchValues?.push(branchValue);
      }

      if (isMultiConfig) {
        if (Array.isArray(configValue)) selectedConfigValues?.push(...configValue);
      } else if (configValue) {
        selectedConfigValues?.push(configValue);
      }

      if (localeValue) {
        if (Array.isArray(localeValue)) selectedLocaleValues?.push(...localeValue);
  ...
  const { selectedBranchValues, selectedConfigValues, selectedLocaleValues } = selectedValues;
  ...
  const renderRow = (mapping: RuleContainerMapping, index: number) => {
    ...
    const rowConfig = typeof configValue === 'string' ? configValue : (Array.isArray(configValue) ? configValue[0] : undefined);
    const isInvalidConfig = Boolean(rowConfig) && validConfigs instanceof Set && !validConfigs.has(rowConfig as string);

    let disabledLocales: any[] = [];
    if (branchValue) {
      // Normalize possible array of branches
      const branches = Array.isArray(branchValue)
        ? branchValue
        : [branchValue];
    ...
    const filteredLocaleOptions = isLocaleExhaustive
      ? localeOptions?.filter((opt) => {
        ...
        const isLocaleUsedForSameBranch = mappings?.some((otherMapping, otherIndex) => {
          if (otherIndex === index) return false; // Skip current row

          const otherBranch = otherMapping?.left ?? otherMapping?.branch_uid;
          const otherLocales = otherMapping?.locales_uid ?? otherMapping?.middle;
    ...
    const filteredConfigOptions = isConfigExhaustive
      ? configOptions?.filter((opt) => {
        ...
        if (isCurrentBranchLevel && branchValue && hasBranchLevelRule(mappings, branchValue, index)) {
          return false;
        }

        // Check if this config is already used at branch level for the same branch
        const isConfigUsedAtBranchLevelFlag = isConfigUsedAtBranchLevel(mappings, branchValue, opt?.value, index);
        ...
        const isDuplicateBranchLevelConfig = mappings?.some((otherMapping, otherIndex) => {
          if (otherIndex === index) return false; // Skip current row

          const otherBranch = otherMapping?.left ?? otherMapping?.branch_uid;
          const otherLocales = otherMapping?.locales_uid ?? otherMapping?.middle;
          const otherConfig = otherMapping?.config_label ?? otherMapping?.right;
          ...
          return isSameBranch && isSameConfig && isCurrentBranchLevel && isOtherBranchLevel;
        });
        ...
        const isConfigUsedForSameBranch = mappings?.some((otherMapping, otherIndex) => {
          if (otherIndex === index) return false; // Skip current row
      ...
      <div key={`pair-${index}`} className={containerClass}>
          ...
          <>
            ...
            <div className="select-wrapper middle-select-wrapper" style={{ width: selectWidth }}>
              <Select
                ...
                noOptionsMessage={() => noOptionsMessage}
                menuPortalTarget={document.body}
                isDisabled={!branchValue || !localeOptions || localeOptions.length === 0}
              />
            </div>
          </>
        )}
        ...
        {showDeleteIcon && (
          <div className="action-icons">
            ...
            {isInvalidConfig ? (
              <Tooltip content={"This configuration was removed. This rule will not work."} position="top" showArrow={false}>
                <Icon
                  icon="WarningBold"
                  size="small"
                  version="v2"
...
function sameBranch(current: string | string[] | null, other?: string): boolean {
