  ...
  ({ src, alt }) => {
    ...
    return (
      <div className="image-container">
        ...
        <div
          className={`asset-icon fallback-icon ${
            isLoaded ? "image-loaded" : ""
          } ${shouldAnimate ? "with-transition" : ""}`}
          aria-hidden="true"
        >
          <Icon icon="Document" size="small" version="v2" />
        </div>
        ...
        <img
          ...
          src={src}
          alt={alt}
          className={`asset-thumbnail ${isLoaded ? "image-loaded" : ""} ${
            shouldAnimate ? "with-transition" : ""
          }`}
          onError={handleError}
          onLoad={handleLoad}
...
function Table({
  ...
  const [selectedRowIdsMap, setSelectedRowIdsMap] = useState<
    Record<string, boolean>
  >({}); // map of selected row IDs for table component
  ...
  const renderAssetIcon = (asset: AssetData) => {
    ...
    if (assetType === "image" && asset?.thumbnailUrl) {
      return <ImageWithFallback src={asset?.thumbnailUrl} alt={asset?.name} />;
    }
  ...
  const fetchData = async ({
    ...
    try {
      ...
      const queryParams = `mode=getAllAssets&location=${
        UI_LOCATIONS.SELECTOR_PAGE
      }&limit=${currentLimit}&skip=${currentSkip}&config=${encodeURIComponent(
      ...
      setError({
        isErr: true,
        errorText: localeTexts.SelectorPage.table.errors.failedToLoadAssets,
      });
      updateLoading(false);
    }
  };
  ...
  const getSelectedRow = (
    ...
    const currentPageAssetIds =
      data?.map((asset: any) => getAssetId(asset)) || [];
    ...
    const newAssets =
      selectedDataAssets?.filter((newAsset: any) => {
    ...
    allSelectedIds?.forEach((assetId: string) => {
      ...
      const existsInArray = updatedAssets?.some(
        (asset: any) => getAssetId(asset) === assetId
      );
      if (!existsInArray) {
        // Try to find it in current page data first
        const assetFromCurrentPage = data?.find(
          (asset: any) => getAssetId(asset) === assetId
        );
        if (assetFromCurrentPage) {
          ...
          const assetFromPreviousSelection = selectedAssets?.find(
            (asset: any) => getAssetId(asset) === assetId
          );
  ...
  useEffect(() => {
    ...
    const selectedAssetIdsChanged =
      JSON.stringify(prevSelectedAssetIds) !== JSON.stringify(selectedAssetIds);
    ...
    if (selectedAssetIds?.length) {
      ...
      const preselectedAssets =
        data?.filter((asset: any) => selectedAssetIds?.includes(asset?.id)) ||
        [];
  ...
  useEffect(() => {
    ...
    if (missingAssets?.length > 0) {
      setSelectedAssets((prevAssets) => {
        ...
        const newAssets = missingAssets?.filter(
          (asset: any) => !existingIds?.includes(getAssetId(asset))
        );
        return [...(prevAssets || []), ...newAssets];
      });
    }
    ...
    const assetsToRemove =
      selectedAssets?.filter((asset: any) => {
        const assetId = getAssetId(asset);
    ...
    <div className="default-table-selector">
      ...
      <div className="button-group">
        ...
        <Button
          ...
          style={{
            backgroundColor: !selectedAssets?.length ? "#ccc" : "#6366f1",
            cursor: !selectedAssets?.length ? "not-allowed" : "pointer",
          }}
        >
          <svg
            width="12"
          ...
          {(() => {
            ...
            return `${localeTexts.SelectorPage.table.buttons.add} ${count} ${
              count !== 1
                ? localeTexts.SelectorPage.table.buttons.addAssets
                : localeTexts.SelectorPage.table.buttons.addAsset
            }`;
          })()}
        </Button>
      </div>
